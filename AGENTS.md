...
- The site reads committed JSON from `site/public/data/{incidents,meta}.json`. To (re)build that data offline from the committed seed, run `.venv/bin/apd seed` (loads `incidents.json` into a fresh SQLite DB at `data/incidents.sqlite`) then `.venv/bin/apd export`. `data/*.sqlite` is gitignored and auto-created.
